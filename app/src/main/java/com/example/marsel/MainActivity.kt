...
class MainActivity : ComponentActivity() {
    ...
    private fun handleServiceTxtRecord(record: Map<String, String?>) {
        try {
            ...
            // Decode the "c" field: comma-separated mobile numbers carried for offline relay chain.
            // Phone B (with network) uses these to forward SMS to Phone A's contacts (e.g. Phone C)
            // even when Phone A has no internet and contacts can't travel via any other path.
    ...
    // Key "c" carries comma-separated mobile numbers for the offline relay chain:
    // when Phone B receives this via DNS-SD it can forward SMS to Phone A's contacts
    // even though Phone A has no internet.
    private fun buildTxtRecord(shortType: String, json: String): Map<String, String>? {
        ...
        // Extract up to 5 mobile numbers from the contacts array
